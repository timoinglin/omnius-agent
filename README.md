...
| 🌐 **playwright** | Headless browsing + a polite parallel crawler, and `weblogin` for sites you register: the **tool** holds the password (from `.env`, which desks may not read) and hands the desk a signed-in session — **a 6-digit 2FA code is asked for in your channel** and used once. It understands two-page forms (email → Next → password), and says plainly when a site hands its login to a corporate identity provider, which no script should try to beat. Or skip scripting entirely: sign in yourself and let desks drive your real browser through the Chrome extension |
| 🖥 **browser** | Opens *your* Chrome on the profile you configured, so a desk can reach a logged-in site when the browser is closed — no profile picker, which from a phone would be a click nobody can make |
...
- **Updates cannot strand the fleet.** `!update go` rebases whatever you changed onto the release and is gated by the full suite — judging only failures the update *introduces* (named, and rolled back), never a machine's own housekeeping; a suite that crashes counts as a failure, never as green — and the reload ends with a handshake: the new watchdog reports back healthy, or the previous commit takes back over **on its own** and says so. Your uncommitted edits are stashed before any rollback and put back after. The update runs on a worker thread, so the bus keeps delivering meanwhile. Every release ships `SHA256SUMS`; `get.ps1` verifies size and hash before unpacking.
- **Secrets stay put.** A hook on every desk blocks any shell, read or search that names `.env` or a saved browser login, and outbox attachments may only come from the media archive, the desk's own folder or its scratchpad — never from `config\`, `state\web` or `.claude\`. A tool folder without a permission file is a library: the bus refuses to run it or mail it, naming the folder.
...
- **1,600+ checks gate every push**, and the docs keep an honest ledger of the difference between what the suite proves and what only live machines caught.
