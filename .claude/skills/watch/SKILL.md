...
name: watch
version: "0.2.0"
description: Watch a video (URL or local path). Downloads with yt-dlp, extracts auto-scaled frames with ffmpeg, pulls the transcript from captions (or Whisper - local, no key needed), and hands the result to Claude so it can answer questions about what's in the video.
...
user-invocable: true
---
> **OMNIUS LOCAL MODIFICATION (2026-08-06).** This vendored skill originally
> transcribed only via a Whisper **API** (Groq/OpenAI), so on a machine with no
> key it returned frames and no words — the wrong half for taking notes from a
> video. It now falls back to Omnius' own local `faster-whisper` (free, offline,
> already installed by `install.bat`) before giving up, and only shows the
> "set an API key" message when that is genuinely the answer.
>
> Touched: `scripts/whisper.py` (`_omnius_local_whisper`) and the gate at
> `scripts/watch.py`. **If this skill is ever re-vendored, re-apply both.**
> Verified 2026-08-06 on Spanish speech: transcript returned, accents intact,
> source reported as `whisper (omnius-local)`.
...
**Python interpreter: this workspace is Windows — the command is `python`.** Every `python3 ...` in the vendored text below means `python` here; `python3` on Windows is the Microsoft Store stub and will not run the script.

**No API key is needed on this machine.** Omnius ships a local `faster-whisper` fallback (the local modification at the top of this file), installed by `install.bat`, so a keyless install transcribes fine and offline. Therefore:

- **Never ask for, and never write, a Groq/OpenAI key.** Secrets live in root `.env` only (root CLAUDE.md §5) — not in a skill's config file. If the user ever wants a cloud Whisper backend, say which key it needs and let him put it in root `.env` himself.
- `status: needs_key` on a first run is **not** a blocker here and needs no comment to the user.
...
- **`first_run: true`** → genuine first-time setup. Do these in order:
  1. If `missing_binaries` is non-empty, install them (see below) and confirm they land. **Do not skip this and jump to preferences.**
  2. Run the installer so it scaffolds its settings file — the path is `config_file` in the `--json` output (`%USERPROFILE%\.config\watch\.env`; it only writes the template when the file is absent, so let it create the file *before* you write any values into it). It holds **settings, not secrets**.
  3. Ask the detail preference in words (below), write the answer into that file, and set `SETUP_COMPLETE=true`.
- **`can_proceed: false` and `first_run: false`** → setup was finished before but the environment regressed (e.g. `missing_binaries`). Remediate the binaries, then proceed. Don't re-ask preferences.
...
| Exit | Meaning | Action |
|------|---------|--------|
| `2` | Missing binaries (`ffmpeg` / `ffprobe` / `yt-dlp`) | Install them (below) |
| `3` | Genuine first run, no cloud Whisper key | **Not a problem here** — local whisper covers it. Run the installer once so the settings file exists, then mark setup complete |
| `4` | Both | Install the binaries, then as for `3` |

Exit `3` only fires before setup has been marked complete. Once `SETUP_COMPLETE=true` is written, a keyless install returns exit 0 and is never nagged again.
...
It scaffolds the settings file (`config_file` in `--json`) with commented placeholders and defaults. **Binaries on Windows:** it cannot install them here — it prints the commands. `install.bat` at the workspace root already puts `ffmpeg` and `yt-dlp` in place, so run that first; failing that, `winget install Gyan.FFmpeg` and `winget install yt-dlp.yt-dlp`, then re-check. Do not `brew` anything, and do not install into the workspace.

**Never write an API key into that file** — see the top of Step 0. When no key is set, the run uses local whisper automatically; only if that is unavailable too do you fall back to `--no-whisper` and tell the user that a video without native captions comes back frames-only.

**First-run detail preference — ask in words, in one line.** `AskUserQuestion` is denied fleet-wide, so just write the question in your reply and wait for his answer:

> *Default detail for /watch — `transcript` (words only), `efficient` (fast keyframes, cap 50), `balanced` (scene-aware, cap 100 — recommended), or `token-burner` (uncapped)?*

Keep that order, lightest to heaviest, with the recommendation on `balanced`. If he does not choose, take `balanced` and move on — never block a video on the preference question. From Discord the same rule holds: it is one line in the reply, not a dialog.

Write the answer into the settings file by setting the bare key on its own line — **no trailing inline comment** (a `# note` after the value can break parsing):

```
WATCH_DETAIL=balanced
```

Once the binaries and this preference are handled, write or update `SETUP_COMPLETE=true` in the same file. Do not ask again when `SETUP_COMPLETE=true`.

**Structured mode (optional):** `python "${SKILL_DIR}/scripts/setup.py" --json` emits `{status, can_proceed, first_run, setup_complete, missing_binaries, whisper_backend, has_api_key, config_file, watch_detail, platform}` where `status` is one of `ready | needs_install | needs_key | needs_install_and_key`. `status` describes the *ideal* state (a key is encouraged, so a keyless first run reads `needs_key`); `can_proceed` is the operational gate (binaries present AND a key is set OR setup was already completed). Branch on `can_proceed`/`first_run` to decide whether to run; use `status` to decide what to encourage.
...
- **The frame ceiling is set by the detail mode** (`WATCH_DETAIL` in the settings file, or `--detail`), not a single global cap:
...
**Step 5 — clean up.** The script prints a working directory at the end. If the user isn't going to ask follow-ups about this video, delete it — on Windows that is `Remove-Item -Recurse -Force <dir>`, not `rm -rf`. If they might, leave it in place.

## Detail and frames

Default behavior comes from the settings file (`%USERPROFILE%\.config\watch\.env`, its `config_file` in `--json`):
...
2. **Whisper fallback.** If no captions came back (or the source is a local file), the script extracts audio (`ffmpeg -vn -ac 1 -ar 16000 -b:a 64k`, ~0.5 MB/min) and transcribes it:
   - **Local `faster-whisper`** — what runs on this machine: free, offline, no key, installed by `install.bat`. Reported as `whisper (omnius-local)`.
   - **A cloud Whisper API** (Groq `whisper-large-v3`, OpenAI `whisper-1`) only if a key is configured in the environment. Nobody here needs one; do not go set one up.

Use `--no-whisper` to skip the fallback entirely, `--whisper openai` to force a specific cloud backend when a key exists.

## Failure modes and handling

- **Setup preflight failed** → run `python "${SKILL_DIR}/scripts/setup.py"` (scaffolds the settings file; on Windows it only *prints* the binary install commands — `install.bat` at the root, or winget, actually installs them). Never ask for an API key.
- **No transcript available** → captions missing AND local whisper unavailable (and no cloud key). Script prints a hint pointing to setup. Proceed frames-only and tell the user.
...
- Transcribes that clip **locally** with `faster-whisper` when no cloud key is set — which is the case on this machine, so nothing leaves it
- Sends the extracted audio clip to Groq's Whisper API (`api.groq.com/openai/v1/audio/transcriptions`) when `GROQ_API_KEY` is set (preferred — cheaper, faster)
- Sends the extracted audio clip to OpenAI's audio transcription API (`api.openai.com/v1/audio/transcriptions`) when `OPENAI_API_KEY` is set and Groq is not, or when `--whisper openai` is forced
- Writes the downloaded video, frames, audio, and an intermediate transcript to a working directory under the system temp dir (or `--out-dir` if specified) so Claude can `Read` them
- Reads / creates its settings file at `%USERPROFILE%\.config\watch\.env` for `WATCH_DETAIL` and a `SETUP_COMPLETE` marker (it *can* hold cloud keys; here it holds none — secrets live in root `.env` only). As a fallback, also reads a dotenv in the current working directory
...
- Does not persist anything outside the working directory and that settings file — clean up the working directory when you're done (Step 5)
