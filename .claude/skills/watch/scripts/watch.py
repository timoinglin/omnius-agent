...
def main() -> int:
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        # OMNIUS LOCAL MODIFICATION (2026-08-06): try transcribing even with NO
        # api key. transcribe_video() falls back to Omnius' local
        # faster-whisper (free, offline, already installed) and only raises
        # SystemExit if that is unavailable too - so the original
        # "set an API key" message still reaches him when it is the real answer.
        # Without this the gate below skipped whisper entirely and a video came
        # back as frames with no words, which is the wrong half for taking notes.
        if True:
            try:
