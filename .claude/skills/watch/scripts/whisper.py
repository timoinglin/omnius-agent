...
def _omnius_local_whisper(video_path, audio_out):
    """-> [{start,end,text}] via local faster-whisper, or None if unavailable.

    OMNIUS LOCAL MODIFICATION. Deliberately tolerant: any failure returns None
    so the caller falls back to its original "set an API key" message rather
    than replacing a clear error with a confusing one.
    """
    try:
    ...
    try:
        ...
        size = os.environ.get("WHISPER_MODEL", "base")   # same knob tools\whisper uses
        print(f"[watch] no API key - transcribing locally with faster-whisper "
        ...
        print(f"[watch] local transcript: {len(out)} segment(s), "
              f"language={getattr(info, 'language', '?')}", file=sys.stderr)
        return out or None
    except Exception as e:
        print(f"[watch] local whisper failed ({type(e).__name__}: {e})",
              file=sys.stderr)
        return None
...
def transcribe_video(
    ...
    if not backend or not api_key:
        # ---- OMNIUS LOCAL MODIFICATION (2026-08-06) -------------------------
        # This vendored skill only speaks CLOUD Whisper, so on a machine with
        # no API key it returned frames and no words - the wrong half for
        # taking notes from a video. Omnius already ships local faster-whisper
        # (tools\whisper\), free and offline, so use that before giving up.
        # If this skill is ever re-vendored, this block is the thing to re-apply.
        local = _omnius_local_whisper(video_path, audio_out)
        if local is not None:
            return local, "omnius-local"
