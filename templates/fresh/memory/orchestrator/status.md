...
- **Shared tools that ship with this install** (`tools\<name>\`, each with a README — read it, don't guess): `whisper` (speech→text, local), `documents` (PDF text + OCR), `transcribe` (recordings → transcript + frames + summary, its own desk), `email` (IMAP/SMTP **and** Microsoft Graph), `playwright` (headless browsing), `remotion` (video rendering). **Playwright is for public pages only — anything behind a login belongs in the Claude Chrome extension, never a scripted login.**
