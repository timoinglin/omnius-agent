...
`!cron` · `!model` · `!screen` · `!desktop` · `!update` · `!trace`
...
- `!update` — self-update from the GitHub repo: bare = fetch + preview what's new; `!update go` = ff-only pull → **full suite (red rolls the update back)** → hooks/permissions restamped → reload → **handshake**: the new watchdog posts ✅ once it proves healthy, and one that crash-loops or sits deaf **reverts to the previous commit on its own** (`state\watchdog\update-pending.json` is the handoff). Refuses on a dirty tree; personal files are gitignored and never move.
- `!trace [id]` — one chain's/loop's/envelope's whole lifecycle from state alone (ledger + gate + loop files): every hop with timestamps, spent-vs-free labels, gate holds with deadlines, budget. Bare = recent chains and loops. The mirror lines print the thread ids this drills into.
