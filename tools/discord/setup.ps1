...
Write-Host '    - New Application -> Bot tab -> "Public Bot" OFF'
Write-Host '    - "Message Content Intent" ON -> press SAVE CHANGES (the green bar -'
Write-Host '      leave without saving and the switch silently resets; the bot then'
Write-Host '      receives every message EMPTY. This step is verified live below.)'
Write-Host '    - Reset Token -> copy it  (that is DISCORD_BOT_TOKEN)'
...
Write-Host '    -> open the generated URL, pick your server, Authorize'
Write-Host ' 4. Fill these in .env:  DISCORD_BOT_TOKEN, DISCORD_GUILD_ID, DISCORD_OWNER_ID'
Write-Host '    (IDs: Settings -> Advanced -> Developer Mode on, then right-click the'
Write-Host '    server name / your own name -> Copy ID)'
Write-Host ''
