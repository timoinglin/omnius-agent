...
### 3.1 Orchestrator (main agent) — persona: the name he gave it (**Omnius** by default)
...
2. On a new message: **enforce the sender allowlist** (`DISCORD_OWNER_ID` — the single security chokepoint, §7) → download attachments to `media\inbox\` → write an envelope into `state\inbox\<target>\`, stamped with **who wrote it**.
   ...
   - **A second writer, on purpose (2026-08-18).** `tools\telegram\bridge.py` is the only other process that composes envelopes: someone with **no Discord account** writes to a Telegram bot, the bridge posts their message into one configured channel and writes the matching envelope itself, reusing the **Discord message id** as the envelope id so ordering, transcripts and `!trace` stay one story. It needed no watchdog change because three properties already held: a bot-authored message is skipped at the first branch (`skip-bot`), so mirroring cannot double-deliver; `ensure_runners` starts a desk from the inbox alone; and `is_human_sender` treats any non-fleet `from` as a guest, so the label *is* the restriction. The allow-list is `config\telegram.ini`, fails closed, and an unlisted id is ignored in silence. The bridge is a **child of the watchdog** (`ensure_telegram_bridge`, started within a minute of the config appearing and restarted when it dies or stops stamping its beacon), not a third scheduled task — a task would mean inviting someone required running a command on the machine, which is not something a fleet driven from a phone can ask for. It holds a pid-validated lock (machine-wide, keyed by the bot token), so a second copy started by anyone parks in **standby** — alive, polling nothing — rather than splitting the bot's mail; it never exits on purpose, because to the watchdog an exit and a crash look the same. Which desk answers is **not** configured: the bridge asks `build_map` — the same channel→session map every owner message goes through — so an invite names a channel and nothing else (`desk =` exists only to override it, or for a channel that maps to no desk, which relays nothing rather than posting something nobody will answer). The chokepoint property is unchanged: control verbs, gate/2FA/takeover answers and slash commands are all inside `if sender == "owner":`, which no bridge can reach.
...
4. **Control commands** it executes itself — no session involved, so they work even when a session hangs, confused or mid-loop: `!kill` (end the channel's session), `!restart` (kill + fresh spawn), `!status` (liveness from claims), and from his own channel only — resolved by **identity** (the desk's pin), never by the name, because that channel is named after the agent and he may rename it: `!killall` — the fleet-wide red button. Kills are always cheap: every project is a git repo, so the worst case lost is a conversation turn, never work.
...
**Work in progress announces itself too (2026-09-02).** The watchdog has two paths that watch a desk holding mail: `check_backlogs()` for a headless run, `interactive_busy()` for a person's own terminal turn. Both used to fall silent exactly when the desk was *working* — the headless one said "still working" once at minute 15 and never again, and the terminal one said nothing at all unless the conversation had gone quiet, in which case its only message offered **`!restart`**. From a phone a long task and a dead desk are indistinguishable, so the silence reads as death and the remedy on offer destroys the work. Now both share one helper: while the evidence says working — the conversation file moving inside `STUCK_QUIET_SECONDS`, no permission pending — it shows `⏳ <desk> is still working (Nm) · last: <tool> <target>` and refreshes it **every `[bus] working_notice_minutes`** (default 3, `0` disables), stopping the moment the desk posts anything of its own, because an ack already explains the wait. `!restart` wording is kept for the three cases that genuinely reach it: a silent turn, a pending dialog, and a turn whose activity cannot be measured at all.

It is **one message, edited in place** (`api.edit_message`), not one per interval: reposting stacked seven near-identical lines above the answer on a twenty-minute turn, which is the channel noise the notice exists to prevent, arriving in another shape. A failed edit falls back to a new message, so a deleted one cannot silence it for good. The *last tool call*, read from the transcript, is what makes the line judgeable from a phone — "3 minutes" alone describes a desk thinking and a desk wedged equally well. No transcript, an unreadable one, or a turn that has not called a tool yet costs the detail and never the notice.

**A turn that stops saying anything is restarted (2026-09-04).** The busy stamp used to be the one signal every remedy trusted: while it stood, no headless run started (a live turn owns the desk), the nudge cooled down, and the still-working line refreshed forever. A desk that wrote its last tool call and then nothing — no error, no reply, no exit — therefore held its mail until the owner restarted it by hand, two hours later. `recover_hung_turns()` now measures idle from the newest tool call in the transcript; past `[delegation] hung_turn_minutes` (default 20, `0` disables) the desk is restarted and the still-working notice names that deadline. Idle starts at the **busy stamp** when the stamp is newer than that call, because right after a new prompt the newest tool call is the *previous* turn's last one — four desks were restarted two seconds into a fresh turn before that distinction existed. A turn that never calls a tool past the limit is still restarted.

**Fleet notices never follow a bridged envelope out (2026-09-04).** A support desk that bridges a public guild receives community messages with `source: "wl"` and the real thread id in `channelId`. That id is where the *desk* answers a person; it is not where the *fleet* talks about itself. `foreign_origin()` marks any envelope a bridge stamped with a source of its own, and `notice_channel_id()` sends every backlog, still-working and `!stop` line about it to the desk's own fleet channel or `#alerts` — before this, desk internals, shell paths and control verbs appeared under a stranded member's post in a server full of strangers. `alert_channel_id()` no longer falls back onto a bridged thread, and `fleet_only_channel()` keeps the loop / workflow "used its budget" line inside a channel the guild map knows, the rule `resolve_outbox_target` already applied to replies.

**The two ends of one message: 👀 and ✅.** The watchdog reacts 👀 the moment an owner envelope is written, and remembers which message that desk owes an answer; when the desk's reply is actually *posted* out of the outbox, it adds ✅. The tick hangs off the successful post rather than off "a reply was written", because a reply that failed to send is not an answer and a ✅ on it would be the most confident possible lie.

**When the fault is Claude's, not ours (2026-09-03).** `conversation_api_error()` reads the last assistant block of a desk's own transcript; when it is an `API Error` and has been still for `API_ERROR_QUIET_SECONDS`, `check_api_outage()` posts once to that desk's channel — the code, and that there is nothing to do — and once more when the conversation moves on with something that is not an error, which is the only honest "back". Detection deliberately does **not** use the `.busy` stamp: `turn_busy()` clears that stamp the instant it sees the error, so anything asking one poll later has nothing left to read.

`api_outage()` then gates the *remedies*, which is the half that matters. During an outage `bridge_not_delivering()` returns False, so no healthy window is replaced to fix a remote API, and the deadman names the API instead of advising `!restart`. Before this, an 85-minute outage killed and reopened six windows and sent the owner restarting healthy desks.

**A window only survives a genuine boot failure.** `desk_bridge.py` exits `BOOT_FAILURE = 3` for the two cases where the window holds the only copy of why (no desk folder, no pty), and `open_tab` pauses on exactly that code — `if errorlevel 3 if not errorlevel 4`, cmd's way of writing `== 3`. Any other exit closes its window, so a `!kill`, a `!restart` or a bridge replaced during an outage no longer leaves a dead "Press any key" tab behind.

**Mail that arrives mid-turn.** A desk already working cannot be handed mail — the watchdog will not start a `--continue` run against a live turn, and the bridge's nudge is keystrokes that land only when the CLI reads again. `mail_notice_hook.py` is a **PostToolUse** hook: what it prints as `additionalContext` reaches the model *between tool calls*, inside the turn already running, so `/omnius`'s "re-drain between major steps" became something a session can actually act on. PostToolUse and not PreToolUse on purpose — a Pre hook can block the call it precedes, and a lost notice is a far better worst case than a wedged turn. One directory listing, no envelope reads, and rate-limited to one notice per count per minute, because a long turn makes hundreds of tool calls.

The bridge's own nudge is bounded the same way: three fast retries per unread count (the measured case is a nudge typed into a session that is not ready yet, which resolves in seconds), then one a minute. Any change in the count is a new reason and resets the budget. Unbounded, it typed 1,577 times into a session that could not answer.

**The machine itself (2026-09-03).** Every other alarm here watches a desk, a chain or an API; none can see the failure that takes all of them down together. `check_system_limits()` samples once a minute — RAM over 90%, free disk under 5 GB or 5%, CPU over 95% *sustained* five minutes, GPU memory over 95% via `nvidia-smi` — and posts one line to `#alerts` per condition, again only after it has recovered and relapsed. `psutil` is optional: without it this check is off and nothing else changes.

**An alarm about a desk goes where someone will see it (2026-09-02).** Every mechanical alarm resolves its channel through `alert_channel_id()`, not `primary_channel_id()` directly. A desk reached only by desk mail *has* no channel of its own, and the old call returned `None` — so `if cid:` silently swallowed the alarm. `tool.discord` failed twelve runs five minutes apart with its permission profile missing, the failure ledger counted every one, and nothing reached anybody: the desk could not speak, which was the fault being reported. The fallback order is its own channel → the **origin channel of the mail it is still holding** (D3: the chain answers the human who started it) → `#alerts`.
...
**What the hooks deliberately do not do: keep anything armed.** Hooks stamp facts (`turn_started`, `.busy`, its clearing), read one string, or hand the model a line of context (`mail_notice_hook`) — and never launch processes — re-arming machinery from a hook was considered on 2026-08-01 and rejected, and the run model then made the question moot: there is nothing left to re-arm.
...
│   ├── #omnius                ← chat with the main agent — named after the agent
│   │                             (#jarvis if you called it Jarvis; #orchestrator still accepted)
│   ├── #fleet-status          ← pinned, always-current fleet overview
│   ├── #transcribe            ← recordings → transcript + frames + summary (own desk)
...
Rules: category name = project name; channel name = component; channel topic = `path | machine | started` (self-healing mapping — if `state\` is lost, the structure re-derives from Discord **by name for one round**: the pins are machine-local and `state\` never travels in the zip, so a channel you had renamed comes back unmapped and says so out loud instead of going quiet, and renaming it back re-pins it). **Those names decide a channel's desk exactly once.** From then on the desk is pinned to the channel's **id** (`state\watchdog\channels.json`), so the owner may rename any channel in the Discord app — `#web` → `#frontend`, `#omnius` → `#maikel` — without a desk going deaf and without the next structure stamp recreating the old name beside it; a *deleted* channel drops its pin and is recreated as before. The orchestrator's channel is named after the agent (`config\omnius.ini`, `[omnius] name`, asked at install): only the address changes, never the folder, the repo or the `/omnius` skill. **One host machine per project at a time** — the topic's machine field is the lock; moving a project = archive on A → pull/unzip on B → spawn on B.
...
- **The watchdog is the single enforcement chokepoint:** only messages from `DISCORD_OWNER_ID`, from a guest inside the channels `config\guests.ini` names for them, or from someone invited in `config\telegram.ini` (whose envelope the bridge writes itself) become envelopes — other users, bots, webhooks are ignored before any session sees them.
...
7. **`.env` never travels in a RELEASE (2026-07-23, scoped 2026-09-01):** `-Fresh` and `-Work` ship only `.env.example` — `pack.ps1` enforces it, and each new instance fills its own (Discord bot values are per-instance, entered when that instance is set up, not during prep). The rule was global until the owner pointed out it also gutted his own backup: *"a full OMNIUS folder backup as is, otherwise it is not a backup."* A **backup** now carries the whole folder — `.env`, `state\`, key files — and only skips what a command rebuilds. Two products, two rules (`docs\BACKUP.md`).
