...
3. **Deliver or start:** if the target session is alive (claim file + PID check, §3.5) its inbox watcher picks the envelope up; otherwise the watchdog spawns the session (§5.3) — resuming the previous conversation if one exists. Bursts queue in the inbox; the claim check prevents double-starts.
...
`from` is the **origin tag**: `owner`, `omnius`, `heartbeat`, `schedule`, a **guest label** from `config\guests.ini` (2026-08-12), or a session id. Anything that is not one of the fleet's own tags (`omnius`/`heartbeat`/`schedule`) is a **person waiting for an answer**, which is what the guards that open a window or raise the deaf-desk alarm test — stated as an exclusion so a guest added later is covered without editing the predicate. A session ignores envelopes with its own origin; instructions from Omnius travel the same visible channel path (post → watchdog → inbox), so delegation is always watchable in Discord. Sessions are Discord-agnostic: any session can be driven in tests by dropping a file into its inbox. The inbox watcher runs as a **background task**, so a fresh envelope surfaces even **mid-task** — steering ("stop", "take this direction", follow-ups) reaches a busy session without waiting for it to finish, just like leaning over at the desk.
...
**Staying reachable — the deaf-desk problem and the `Stop` hook (built 2026-08-01).** A Claude session is **turn-based, not a daemon**: it runs a turn and stops. It stays reachable only because `inbox_watch` runs as a **Bash background task** — when that task exits, the harness re-invokes the session. That re-invocation *is* the wake mechanism. So a turn that ends without re-arming the watcher leaves a desk **alive outside, deaf inside**: the claim still names a live pid, `!status` says `[on]`, and the watchdog keeps queueing envelopes into a box nobody will read.

Two routes reach that conclusion:

| Route | Latency | Covers |
|---|---|---|
| **Stop hook** (`tools\discord\turn_end_hook.py`) writes `state\watchdog\turn_ended\<id>.json` the instant a turn ends with no live `watcherPid` | seconds | desks whose settings register the hook |
| **Stale heartbeat** — a live watcher rewrites its claim every ~5 s, so a frozen `lastSeenAt` + unread mail means deaf | minutes | every desk, hook or not |

`heal_deaf_desks()` takes either route, then **kills before respawning** (two sessions on one desk is the worse failure) and says so in the channel. Both routes still require **unread mail** — a quiet desk with an empty inbox is idle, not broken — and the marker is re-checked against live PID at the moment of acting, because a turn can end microseconds before the session re-arms.

**Finished work announces itself (built 2026-08-01).** `inbox_watch` stamps `state\watchdog\turn_started\<id>.json` when it delivers, because it is the process that knows a turn is beginning. The `Stop` hook reads it at the end: if nothing was posted to the outbox since that moment, it posts a one-liner naming the files that changed (`git status --porcelain`, so `.gitignore` keeps `node_modules`/`dist` churn out of the signal). It speaks only when there is something to answer for — a turn a delivered envelope started, that posted nothing back. This exists because a desk once built a PDF correctly and stopped, and the result died on disk while the owner asked all afternoon whether anything was happening.

**What the hook deliberately does not do: re-arm the watcher.** A process launched from a hook is not one of the harness's background tasks, so it could not wake anything — but it *would* keep the claim's heartbeat fresh, which is exactly the signal `heal_deaf_desks()` reads as "this desk is fine". Helpfully re-arming from the hook would mask deafness and disable the safety net. It makes recovery faster; it does not pretend to fix it. (Considered and rejected 2026-08-01.)
...
|---|---|---|
| `/omnius` | any session | plug this session into the bus: write claim, start inbox watcher, hello-post via outbox |
...
Identity from cwd → write claim file → start inbox watcher (background task; a new envelope wakes the session) → hello via outbox: *"🟢 online: recipe-app/app @ main-pc"*.
