...
Rules: category name = project name; channel name = component; channel topic = `path | machine | started` (self-healing mapping — if `state\` is lost, everything re-derives from Discord itself). **Those names decide a channel's desk exactly once.** From then on the desk is pinned to the channel's **id** (`state\watchdog\channels.json`), so the owner may rename any channel in the Discord app — `#web` → `#frontend`, `#omnius` → `#maikel` — without a desk going deaf and without the next structure stamp recreating the old name beside it; a *deleted* channel drops its pin and is recreated as before. The orchestrator's channel is named after the agent (`config\omnius.ini`, `[omnius] name`, asked at install): only the address changes, never the folder, the repo or the `/omnius` skill. **One host machine per project at a time** — the topic's machine field is the lock; moving a project = archive on A → pull/unzip on B → spawn on B.
